document.addEventListener("DOMContentLoaded", () => {
    ...
    if (uploadedPhotos) {
      ...
      // 過濾掉「與當前 album 相同」的照片
      albumPhotos = uploadedPhotos.filter(p => p.album === album);
    }
  
    // 如果該相簿下根本沒有多張照片，至少包含當前這張
    // 也可考量：若 albumPhotos 找不到 photoUrl，就把它新增進去
    ...
    // 10. 「相片詳細表單」提交事件 → 更新 localStorage + 寫入 Supabase
